...
import main
from conftest import ADMIN, USER, get_user_login, get_basic_auth, get_admin_login, get_user_login_body
...
def test_change_user_pw(testing_db):
    ...
    response = hug.test.post(main, "/login", body=get_user_login_body(password=NEW_PASSWORD))
    assert response.status == hug.HTTP_200
