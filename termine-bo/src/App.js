        ...
        return Api.fetchUsers()
            .then(response => {
                if (response.status === 200) {
                    setUserList(response.data.reduce((acc, user) => {acc[user.user_name] = user; return acc}, {}));
                }
            })
            .catch(error => {
                //TODO: error handling
            ...
            <div className="row">
                <FocusLock group="booking-workflow">
                    <form>
                        ...
                        <input type="text" name="new_user_name" placeholder="Benutzername" value=""/>
                        <input type="password" name="new_user_password" placeholder="Passwort" value=""/>
                        <input type="password" name="new_user_password_confirm" placeholder="Passwortverifikation" value=""/>
                        ...
                        <table style={{maxHeight: "98vh", marginTop: "1vh"}}>
                                        ...
                                        <div className="container">
                                            <div className="row">
                                                ...
                                                <div className="col-lg-6" style={{
                                                    ...
                                                    <span className={showSpinner ? "icon-refresh animated" : "icon-refresh"} style={{
                                                              position: "absolute",
                                                              top: "var(--universal-padding)",
                                                              right: "var(--universal-padding)",
                                                              height: "1.6rem",
                                                              width: "1.6rem"
                                                          }} onClick={refreshList}></span>
                                                </div>
                                            </div>
                                        </div>
                                    </th>
                            ...
                            <tbody>
                                {Object.values(userList).map(user => {
                                    const style = {fontSize: "1.2em", display: "flex", alignItems: "center"}
                                    return <tr className={false ? "active" : ""} key={user.user_name}>
                                               <td style={style}>{user.user_name}</td>
                                               <td style={style}><input id="is_admin" disabled={true} type="checkbox" checked={user.is_admin} onChange={e => patchUser({...user, is_admin: e.target.value || false})}/></td>
                                               <td style={style}>{user.total_bookings}</td>
                                               <td style={style}><input id="coupons" type="number"
                                                                        min="0"
                                                                        value={user.coupons}
                                                                        onFocus={e => setEditedUser(user)}
                                                                        onBlur={saveEditedUser}
                                                                        onChange={patchEditedUser}
                                                                        onKeyDown={onEnterSave}/></td>
                                           </tr>;
                                })}
                            </tbody>
                        </table>
