...
import {DateFormatter, INFOBOX_STATES, ISOStringWithoutTimeZone} from "./utils";
...
function App() {
    ...
    const [selectedAppointment, setSelectedAppointment] = useState({startDateTime: "", lengthMin: 0});
    const [bookedAppointment, setBookedAppointment] = useState({startDateTime: "", lengthMin: 0});
    ...
    const [infoboxState, setInfoboxState] = useState({state: INFOBOX_STATES.INITIAL, msg: ''});
    ...
    const getBookedListData = useCallback(() => {
        return Api.fetchBooked(ISOStringWithoutTimeZone(startDate), ISOStringWithoutTimeZone(endDate))
            .then(response => {
                if (response.status === 200) {
    ...
    const claimAppointment = (startDateTime, slotLengthMin) => {
        ...
        return Api.claimSlot(startDateTime)
            .then(response => {
                ...
                setSelectedAppointment({startDateTime: startDateTime, lengthMin: slotLengthMin})
    ...
    const onBook = (data) => {
        Api.book(data).then((response) => {
            setInfoboxState({
                state: INFOBOX_STATES.APPOINTMENT_SUCCESS,
                msg: response.data.secret,
            });
            setStartDateTime(response.data.time_slot);
            setBookedAppointment({startDateTime: response.data.time_slot, lengthMin: response.data.slot_length_min})
            setClaimToken('');
            setFocusOnList(true);
        }).catch(error => {
            setInfoboxState({
        ...
        <div className="container">
            <header style={{minHeight: "54px", maxHeight: "6vh"}}>
                <div className="displayFlex">
                    <h3 style={{paddingLeft: "var(--universal-padding)"}}>{window.config.longInstanceName}</h3>
                    <a href="#" className="button" onClick={() => setCurrentTab(TAB.BOOK)}>Termine buchen</a>
                    <a href="#" className="button" onClick={() => setCurrentTab(TAB.BOOKED)}>Meine Buchungen</a>
                    <div className="flexAlignRight">
                        <input type="button" value="Logout" onClick={logout}/>
                    </div>
                </div>
            </header>
            {currentTab === TAB.BOOK && BookView(focusOnList, freeSlotList, coupons, claimAppointment, setSelectedAppointment, selectedAppointment, showSpinner, refreshList, infoboxState, bookedAppointment, onBook, onCancelBooking, claimToken, startDateTime, formState, setFormState, inputRef)}
            {
                currentTab === TAB.BOOKED && BookingHistoryView(bookedList, startDate, setStartDate, endDate, setEndDate)
            }
        </div>
    );
}
