...
export const formatDate = (ISOString) => ISOString? format(parseISO(ISOString), 'PP', {locale: de}): '';
export const formatTime = (ISOString, slotLengthMin) => {
    const date = parseISO(ISOString);
    return ISOString? `${format(date, 'p', {locale: de})} - ${format(add(date, {minutes: slotLengthMin}), 
        'p', {locale: de})}`: '';
};
